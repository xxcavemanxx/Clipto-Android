...
import com.wb.clipboard.databinding.BlockMainNavHeaderBinding
...
class HeaderBlock(val viewModel: MainNavViewModel) : BlockItem<MainNavFragment>() {
    ...
    override fun onInit(fragment: MainNavFragment, block: View) {
        val binding = BlockMainNavHeaderBinding.bind(block)
        val appConfig = viewModel.appConfig
        binding.appNameTextView.text = StyleHelper.getAppTitle(block.context)
        ...
        binding.actionSettings.setDebounceClickListener {
            if (appConfig.isRunesEnabled()) {
                fragment.navigateTo(R.id.action_runes)
            } else {
                fragment.navigateTo(R.id.action_settings)
        ...
        binding.actionSettings.setOnLongClickListener {
            if (appConfig.isRunesEnabled()) {
                fragment.navigateTo(R.id.action_settings)
            } else {
                fragment.navigateTo(R.id.action_runes)
