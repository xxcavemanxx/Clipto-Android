...
class TextRoundedBgHelper(
    ...
    fun draw(canvas: Canvas, text: Spanned, layout: Layout) {
        ...
        val spanItems = AppTextCache.getItem(text)?.spans
                ?: text.getSpans(0, text.length, ColorfulTagSpan::class.java)
        spanItems.forEach { span ->
            val spanStart = text.getSpanStart(span).let {
                if (span is ColorfulIconTagSpan) {
                    it - 3
                } else {
                    it
                }
...
//                val renderer = if (startLine == endLine) singleLineRenderer else multiLineRenderer
            ...
            renderer.draw(canvas, layout, startLine, endLine, startOffset, endOffset, span.tagColor)
        }
    }
}